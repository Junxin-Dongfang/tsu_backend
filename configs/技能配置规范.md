...
### 7. 技能升级消耗配置 (skill_upgrade_costs)
**说明**: 全局技能升级消耗（所有技能共用）

| 列名 | 数据类型 | 必填 | 说明 | 示例 |
|------|---------|------|------|------|
| 等级 | 数字 | ✓ | 升级到第N级 | 2 (表示1级升到2级) |
| 经验消耗 | 整数 | ✓ | 升级所需经验 | 100 |
| 金币消耗 | 整数 | ✓ | 升级所需金币 | 50 |
| 材料消耗 | JSON数组 | - | 所需材料列表 | [{"item_code":"skill_book_basic","count":1}] |
...
### 8. 技能解锁动作关联 (skill_unlock_actions)
**说明**: 定义技能解锁的动作及其等级成长配置

| 列名 | 数据类型 | 必填 | 说明 | 示例 |
|------|---------|------|------|------|
| 技能ID | UUID | ✓ | 关联skills表 | uuid |
| 动作ID | UUID | ✓ | 关联actions表 | uuid |
| 解锁等级 | 数字 | ✓ | 技能达到此等级时解锁 | 1 |
| 是否默认 | 布尔 | - | 是否为默认动作 | true |
| **等级成长配置** | **JSON文本** | **-** | **动作的属性成长规则** | **见下方说明** |

**等级成长配置格式** (level_scaling_config):
```json
{
  "damage": {
    "type": "linear",      // 成长类型: linear/percentage/fixed
    "base": 10,            // 基础值
    "value": 2             // 每级增长值
  },
  "accuracy": {
    "type": "percentage",  // 百分比增长
    "base": 0,
    "value": 1             // 每级+1%
  },
  "cooldown": {
...
**成长类型说明**:
- `linear`: 线性增长。计算公式: `base + (level-1) * value`
- `percentage`: 百分比增长。计算公式: `base * (1 + (level-1) * value / 100)`
- `fixed`: 固定值，不随等级变化
...
2. **强力一击** (5级解锁):
   ```json
   {
     "damage": {"type": "linear", "base": 20, "value": 5},
...
3. **旋风斩** (10级解锁):
   ```json
   {
     "damage": {"type": "percentage", "base": 30, "value": 15},
     "range": {"type": "fixed", "base": 2, "value": 0}
   }
   ```
