...
var ClassAdvancedRequirementWhere = struct {
	...
	FromClassID:            whereHelperstring{field: "\"class_advanced_requirements\".\"from_class_id\""},
	ToClassID:              whereHelperstring{field: "\"class_advanced_requirements\".\"to_class_id\""},
	RequiredLevel:          whereHelperint{field: "\"class_advanced_requirements\".\"required_level\""},
	RequiredHonor:          whereHelperint{field: "\"class_advanced_requirements\".\"required_honor\""},
...
func (o *ClassAdvancedRequirement) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"class_advanced_requirements\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"class_advanced_requirements\" %sDEFAULT VALUES%s"
		}
...
func (o ClassAdvancedRequirementSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"class_advanced_requirements\" SET %s WHERE %s",
...
func (o *ClassAdvancedRequirement) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"class_advanced_requirements\"", updateOnConflict, ret, update, conflict, insert, opts...)
