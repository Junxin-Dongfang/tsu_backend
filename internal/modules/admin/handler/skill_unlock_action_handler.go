...
// @Summary 为技能添加解锁动作
// @Description 为指定技能添加单个解锁动作关联,指定解锁等级和是否为默认动作
// @Tags 动作系统
// @Accept json
// @Produce json
// @Param skill_id path string true "技能的UUID"
// @Param request body AddSkillUnlockActionRequest true "添加解锁动作的请求参数,包含动作ID和解锁等级"
// @Success 200 {object} response.Response{data=SkillUnlockActionInfo}
...
// @Summary 批量设置技能解锁动作
// @Description 批量设置技能的所有解锁动作关联(先删除旧关联,再创建新关联,事务保证)
// @Tags 动作系统
// @Accept json
// @Produce json
// @Param skill_id path string true "技能的UUID"
// @Param request body BatchSetSkillUnlockActionsRequest true "批量设置解锁动作的请求参数,包含动作列表"
// @Success 200 {object} response.Response
// @Failure 400 {object} response.Response "参数错误或验证失败"
// @Failure 404 {object} response.Response "技能或动作不存在"
// @Failure 500 {object} response.Response "服务器内部错误"
