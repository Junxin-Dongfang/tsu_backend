...
type InviteMemberRequest struct {
	TeamID        string  `json:"team_id" validate:"required" example:"team-uuid-001"`        // 团队ID（必填）
	InviterHeroID string  `json:"inviter_hero_id" validate:"required" example:"hero-uuid-001"` // 邀请人英雄ID（必填）
...
func (h *TeamMemberHandler) ApplyToJoin(c echo.Context) error {
	...
	if err := h.memberService.ApplyToJoin(c.Request().Context(), applyReq); err != nil {
...
// InviteMember 邀请成员
// @Summary 邀请成员
// @Description 邀请成员加入团队（需要队长/管理员审批）
// @Tags 团队成员
// @Accept json
// @Produce json
...
func (h *TeamMemberHandler) InviteMember(c echo.Context) error {
	...
	if err := h.memberService.InviteMember(c.Request().Context(), inviteReq); err != nil {
		return response.EchoError(c, h.respWriter, err)
	}
