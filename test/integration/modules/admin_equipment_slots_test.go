...
// 装备槽位列表读取。
func TestAdminEquipmentSlotsList(t *testing.T) {
	ctx, cfg, client, _ := setup(t)
	token := adminToken(t, ctx, client, cfg)

	resp, httpResp, raw, err := apitest.GetJSON[map[string]interface{}](ctx, client, "/api/v1/admin/equipment-slots?page=1&page_size=5", token)
...
func TestAdminEquipmentSlotsListUnauthorized(t *testing.T) {
	ctx, _, client, _ := setup(t)
	resp, httpResp, raw, err := apitest.GetJSON[map[string]interface{}](ctx, client, "/api/v1/admin/equipment-slots?page=1&page_size=5", "")
	require.NoError(t, err, string(raw))
