...
// 怪物列表阅读权限。
func TestAdminMonstersList(t *testing.T) {
	ctx, cfg, client, _ := setup(t)
	token := adminToken(t, ctx, client, cfg)

	resp, httpResp, raw, err := apitest.GetJSON[map[string]interface{}](ctx, client, "/api/v1/admin/monsters?page=1&page_size=5", token)
...
func TestAdminMonstersListUnauthorized(t *testing.T) {
	ctx, _, client, _ := setup(t)
	resp, httpResp, raw, err := apitest.GetJSON[map[string]interface{}](ctx, client, "/api/v1/admin/monsters?page=1&page_size=5", "")
	require.NoError(t, err, string(raw))
